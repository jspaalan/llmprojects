...
export function renderDiagram(model) {
  ...
  const { boxes } = layout.resolveBoxes(model.elements, model.elementMap);
...
function drawGrid(model, layout) {
  ...
  for (let c = 0; c <= model.grid.columns; c++) {
    const x = c === model.grid.columns ? model.canvas.width - layout.padding : layout.point({ c, r: 0 }).x;
    group.append(svgNode("line", { x1: x, y1: layout.padding, x2: x, y2: model.canvas.height - layout.padding }));
  }
  for (let r = 0; r <= model.grid.rows; r++) {
    const y = r === model.grid.rows ? model.canvas.height - layout.padding : layout.point({ c: 0, r }).y;
    group.append(svgNode("line", { x1: layout.padding, y1: y, x2: model.canvas.width - layout.padding, y2: y }));
  }
  return group;
}
...
function drawElement(element, boxes, layout) {
  ...
  const isOutline = element.kind === "outline";
  const isShape = element.kind === "shape";

  group.append(svgNode("rect", {
    x: b.x, y: b.y, width: b.width, height: b.height,
    rx: style(s.radius, isNode ? 12 : 10),
    fill: style(s.fill, isNode ? "#172b46" : isShape ? "#10243a" : "#0f2034"),
    stroke: style(s.stroke, isNode ? "#4f89bd" : "#385879"),
    "stroke-width": style(s.strokeWidth, 2),
    "stroke-dasharray": style(s.dash, isOutline ? "8 6" : null)
  }));

  if (s.titleBar) {
    const titleHeight = style(s.titleBarHeight, 28);
    group.append(svgNode("rect", {
      x: b.x, y: b.y, width: b.width, height: titleHeight,
      rx: style(s.radius, 10), fill: style(s.titleBarFill, s.stroke || "#385879"), opacity: style(s.titleBarOpacity, .22)
    }));
  }
  ...
  if (isNode && element.icon) {
    ...
    const text = svgNode("text", { x: cx, y: cy + 1, class: "icon-text", "font-size": Math.max(8, size * .26) });
    text.textContent = String(element.icon).slice(0, 6).toUpperCase();
    group.append(icon, text);
  }

  if (element.label) {
    const titleAtTop = element.kind === "container" || isOutline || isShape;
    ...
    addLabel(group, element.label, labelX, labelY, element.kind, s);
  }

  if (s.watermark) {
    const watermark = svgNode("text", {
      x: b.x + b.width / 2,
      y: b.y + b.height / 2,
      fill: style(s.watermarkColor, s.stroke || "#7aa2c8"),
      opacity: style(s.watermarkOpacity, .12),
      "font-size": style(s.watermarkSize, Math.min(46, b.width / 6)),
      "font-weight": 800,
      "text-anchor": "middle",
      "dominant-baseline": "middle"
    });
    watermark.textContent = s.watermark;
...
function addLabel(group, label, x, y, kind, s) {
  ...
  const text = svgNode("text", {
    x, y: y - ((lines.length - 1) * 8),
    class: kind === "node" ? "element-label" : `${kind}-label`,
    "font-size": style(s.fontSize, kind === "node" ? 14 : 13),
    fill: s.labelColor,
    "text-anchor": anchor
  });
  lines.forEach((line, i) => {
    const span = svgNode("tspan", { x, dy: i ? "1.2em" : 0 });
...
function drawBadge(element, boxes, layout) {
  ...
  const [ax, ay] = anchors[corner] || anchors["top-right"];
  const x = ax + (offset.c || 0) * (layout.cellWidth + layout.gap);
  const y = ay + (offset.r || 0) * (layout.cellHeight + layout.gap);
  const width = element.width || Math.max(28, String(element.label || "").length * 7 + 16), height = element.height || 24;
  const group = svgNode("g", { "data-element-id": element.id, "data-kind": "badge" });
  ...
  const text = svgNode("text", { x, y, class: "badge-text" });
