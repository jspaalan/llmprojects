...
const svgNode = (name, attrs = {}) => { const node = document.createElementNS(NS, name); Object.entries(attrs).forEach(([k, v]) => node.setAttribute(k, v)); return node; };
...
export function renderDiagram(model) {
  ...
  [...model.layers].sort((a, b) => (a.zIndex || 0) - (b.zIndex || 0)).forEach(layer => {
    const group = svgNode("g", { "data-layer": layer.id || "" });
  ...
  model.connections.forEach((connection, index) => {
    ...
    const path = svgNode("path", { d: orthogonalPath(start, end, waypoints), class: "connection", stroke: style(connection.style?.stroke, "#7dd3fc"), "stroke-width": style(connection.style?.strokeWidth, 2.5), "marker-end": "url(#arrow-end)", "data-connection": connection.id || index });
    if (connection.style?.arrow === "both") path.setAttribute("marker-start", "url(#arrow-start)");
...
function drawElement(element, boxes, layout) {
  ...
  const b = boxes.get(element.id), s = element.style || {}, group = svgNode("g", { "data-element-id": element.id, "data-kind": element.kind });
  const isNode = element.kind === "node", dashed = element.kind === "outline";
  group.append(svgNode("rect", { x: b.x, y: b.y, width: b.width, height: b.height, rx: style(s.radius, isNode ? 12 : 16), fill: style(s.fill, isNode ? "#172b46" : "#0f2034"), stroke: style(s.stroke, isNode ? "#4f89bd" : "#385879"), "stroke-width": style(s.strokeWidth, 2), ...(dashed ? { "stroke-dasharray": "8 6" } : {}) }));
  if (isNode && element.icon) {
    const size = Math.min(42, b.height * .34), cx = b.x + b.width / 2, cy = b.y + b.height * .34;
    const icon = svgNode("rect", { x: cx - size / 2, y: cy - size / 2, width: size, height: size, rx: 9, class: "icon-placeholder", color: style(s.accent, "#67e8f9") });
    const text = svgNode("text", { x: cx, y: cy + 1, class: "icon-text", "font-size": Math.max(9, size * .28) }); text.textContent = String(element.icon).slice(0, 5).toUpperCase(); group.append(icon, text);
  }
  const labelY = isNode && element.icon ? b.y + b.height * .7 : (element.kind === "container" || dashed ? b.y + 20 : b.y + b.height / 2);
  addLabel(group, element.label || element.id, b.x + b.width / 2, labelY, element.kind, s);
  return group;
}

function addLabel(group, label, x, y, kind, s) {
  const lines = Array.isArray(label) ? label : String(label).split("\n");
  const text = svgNode("text", { x, y: y - ((lines.length - 1) * 8), class: kind === "node" ? "element-label" : `${kind}-label`, "font-size": style(s.fontSize, kind === "node" ? 14 : 13) });
...
function drawBadge(element, boxes, layout) {
  ...
  group.append(svgNode("rect", { x: x - width / 2, y: y - height / 2, width, height, rx: height / 2, fill: style(element.style?.fill, "#e11d48"), stroke: style(element.style?.stroke, "#fecdd3"), "stroke-width": 1.5 }));
